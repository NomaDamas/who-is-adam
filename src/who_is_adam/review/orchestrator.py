    ...
    *,
    ...
    try:
        ...
        return ReviewRunResult(
            ...
            refusal=Refusal(
                ...
                diagnostics=[
                    GateResult(status=GateStatus.REJECT, reasons=[f"pdf_extraction_error: {exc}"])
                ],
            ),
            metadata=metadata,
        )
    ...
    if gate.status is GateStatus.REJECT:
        return ReviewRunResult(
            status=ReviewRunStatus.REFUSED,
            refusal=Refusal(
                reason="pre-review quality or prompt-injection gate rejected the PDF",
                diagnostics=[gate],
            ),
            metadata=metadata,
        )

    desk_checks = run_desk_reject_checks(
        paper, supplementary_provided=supplementary_pdf_path is not None
    )
    ...
    if blockers:
        return ReviewRunResult(
            status=ReviewRunStatus.REFUSED,
            refusal=Refusal(
                reason="ICML Main Track desk-reject check failed", diagnostics=list(blockers)
            ),
            metadata=metadata,
        )

    client = llm_client or _llm_client(config)
    citation_checks = verify_paper_citations(paper, config)
    specialist_reviews = run_specialist_reviews(
        paper=paper,
        llm_client=client,
    ...
    markdown = render_review_markdown(
        ...
        appendices={
            ...
            "citation_checks": [item.model_dump(mode="json") for item in citation_checks],
...
def _input_refusal(pdf_path: Path) -> Refusal | None:
    if not pdf_path.exists():
        return Refusal(
            ...
            diagnostics=[
                GateResult(status=GateStatus.REJECT, reasons=[f"input_not_found: {pdf_path}"])
            ],
        )
    if not pdf_path.is_file():
        return Refusal(
            reason="PDF input was refused before extraction",
            diagnostics=[
                GateResult(status=GateStatus.REJECT, reasons=[f"input_not_file: {pdf_path}"])
            ],
        )
    if pdf_path.suffix.lower() != ".pdf":
        return Refusal(
            ...
            diagnostics=[
                GateResult(
                    status=GateStatus.REJECT, reasons=["input_not_pdf: input must be a PDF file"]
                )
            ],
        )
        ...
        return Refusal(
            ...
            diagnostics=[
                GateResult(
                    ...
                    reasons=[
                        f"input_too_large: PDF is {size_bytes} bytes; maximum is {MAX_PDF_BYTES} bytes"
                    ],
                )
            ],
        )
...
def _pdf_extractor(config: ReviewConfig) -> PdfExtractor:
    ...
    try:
        ...
        raise ReviewOrchestrationError(
            f"OCR is enabled but Tesseract OCR is not available: {exc}"
        ) from exc
