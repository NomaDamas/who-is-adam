...
일반 플러그인 사용에는 Python이 필요하지 않습니다. CLI 개발이나 결정적 오프라인 검증을 할 때만 Python 3.11+ 환경에 설치합니다.
...
스킬 진입점은 `skills/who-is-adam/SKILL.md`이며 Claude Code와 Codex용 플러그인 manifest를 함께 제공합니다. 권장 설치 방식은 플러그인이며 Python 패키지는 필요하지 않습니다.
...
플러그인을 지원하지 않는 런타임에서는 스킬 디렉터리를 직접 설치할 수 있습니다.
...
일반 플러그인 사용에는 환경 변수가 필요하지 않습니다. 선택적 CLI 검증에서는 `WHO_IS_ADAM_OFFLINE=true` 또는 `--offline`을 사용하고 `--llm-policy`, `--code-of-conduct-ack`를 명시합니다. Hosted 제공자 환경 변수는 hosted CLI 리뷰 생성을 활성화하지 않습니다.
