...
- Crossref, Semantic Scholar, OpenAlex, arXiv, 공개 OpenReview 근거를 사용하되, 충돌은 `needs_review`로 남기고 확인되지 않은 사실은 추측하지 않는다.
...
- 외부 메타데이터: Crossref, Semantic Scholar, OpenAlex, arXiv는 참고문헌 사실 확인 보조로만 사용하며 충돌은 `needs_review`로 남깁니다.
...
운영 환경 변수, fake/offline 모드, 제공자별 실패 의미는 [운영자 가이드](operator-guide.md)의 `환경 변수 매트릭스`를 따릅니다. 주요 제공자는 LLM, OpenReview, Semantic Scholar, Crossref, OpenAlex, arXiv, 선택적 OCR/Tesseract입니다.
