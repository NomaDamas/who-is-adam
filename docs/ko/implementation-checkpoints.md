...
이 문서는 승인된 단계별 구현 계획을 한국어로 정리합니다. 현재 작업 범위는 체크포인트 1의 문서 다섯 파일뿐입니다. 제품 코드, `.gjc`, 테스트, 포매터, Git 명령은 이 체크포인트에서 실행하지 않습니다.
...
- 그 다음 Python `>=3.11` 기반 skeleton과 schema를 만들고, 공식 ICML 점수 범위를 코드로 검증합니다.
- deterministic PDF fixture와 추출기를 추가한 뒤 안전 게이트와 desk check를 구현합니다.
- 외부 근거 제공자는 no-network 테스트로 검증하며, OpenReview 근거 부재를 추측하지 않습니다.
- 독립 specialist와 synthesis는 prompt boundary를 유지합니다.
- 마지막으로 Markdown 저장, output path versioning, golden output, CLI smoke를 검증하고 문서를 실제 동작과 동기화합니다.
